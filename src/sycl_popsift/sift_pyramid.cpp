  ...
  , _d_consts(d_consts)
{
    ...
    //
    // _d_extrema_num_blocks = popsift::common_sycl::malloc_sharedT<int>(
    //   _num_octaves, __FILE__, __LINE__, "Global octave barrier allocation failed", _device_queue);
    //
    // // don't see the purpose of dobuf_shadow, so not using it untill I see a purpose for it (I probs will in future
    // :D)
...
Pyramid::~Pyramid()
{
    ...
    // fprintf(stderr, "Pointer _d_extrema_num_blocks = %p\n", _d_extrema_num_blocks);
    // sycl::free(_d_extrema_num_blocks, _device_queue); // FAILS why God knows
    // fprintf(stderr, "Pointer _d_extrema_num_blocks = %p\n", _d_extrema_num_blocks);
